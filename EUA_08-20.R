...
# Data processing
###########################################
EUA_euro = read_csv('EUA_08-20.csv') %>%
  janitor::clean_names()
n = nrow(EUA_euro)-1
EUA_euro <- EUA_euro[1:n,c(4,10)]
EUA_euro <- EUA_euro %>% rename(date = time,
         price_euro = last)
...
imfs_adj = melt(subset(imfs,select=c(date,imf1,imf2,imf3,imf4,residue,price)),id.var="date")
...
low_fit = imfs$low_f %>% auto.arima(stationary=T, 
                                    seasonal = T, 
                                    test = c("kpss", "adf", "pp"),  
                                    ic = c("aicc", "aic", "bic"), 
                                    allowdrift = T, 
...
# trend_fit2 <- lm(y~poly(x, 3), data.frame(x=1:n_train, y=imfs$residue))
# trend_prediction2 <- predict(trend_fit2, data.frame(x=111:129), se=T)
...
# plot(imf_pred)
# 
# trend25 <- lm(y~poly(x, 3), data.frame(x=1:110, y=imfs$residue))
# tp25 <- predict(trend25, data.frame(x=111:129), se=T)
# 
